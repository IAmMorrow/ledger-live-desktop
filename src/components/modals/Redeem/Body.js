...
const Body = ({
  ...
  const handleCloseModal = useCallback(() => closeModal(MODAL_REDEEM), [closeModal])
  ...
  const handleSignTransaction = useCallback(
    async ({ transitionTo }: { transitionTo: string => void }) => {
      ...
      signTransactionSubRef.current = bridge
        .signAndBroadcast(mainAccount, {...transaction, erc20Method: 'compoundRedeemDAI'}, device.path)
        .subscribe({
          next: e => {
            switch (e.type) {
              case 'signed': {
