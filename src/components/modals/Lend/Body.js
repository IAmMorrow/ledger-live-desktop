...
import { MODAL_LEND } from 'config/constants'
...
const Body = ({
  ...
  const handleCloseModal = useCallback(() => closeModal(MODAL_LEND), [closeModal])
  ...
  const handleSignTransaction = useCallback(
    async ({ transitionTo }: { transitionTo: string => void }) => {
      ...
      signTransactionSubRef.current = bridge
        .signAndBroadcast(mainAccount, {...transaction, erc20Method: 'compoundMintCDAI'}, device.path)
        .subscribe({
          next: e => {
            switch (e.type) {
              case 'signed': {
